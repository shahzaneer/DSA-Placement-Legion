...
public class CountReversePairs {
    public static void main(String[] args) {
        int [] array = {4, 1, 2, 3, 1};
        System.out.println(countReversePairsBrute(array));
        System.out.println(countReversePairsOptimal(array));
    }

    //TODO: i < j && array[i] > (array[j] * 2) then (array[i],array[j]) is a reverse Pair

    //! Brute: TC: O(n^2) SC: O(1) - trying all possible solutions
    static int countReversePairsBrute(int [] array){
        int cunt = 0;
    ...
    //! Optimal: TC: O(n log n + n) SC: O(n) - merge sort modification
    static int countReversePairsOptimal(int [] array){
    ...
    static int countReversePairs(int [] array, int low, int mid, int high){
        ...
        for(int i=low; i<=mid; i++){
            ...
            while(currentRight <= high && array[i] > (2* array[currentRight])){
